...
static VOID  halBusInit (VOID)
{
    ...
    API_I2cAdapterCreate("/bus/i2c/0", pI2cFuncs, 10, 1);               /*  创建 i2c0 总线适配器        */

    pSpiFuncs = spiBusDrv(0);
    API_SpiLibInit();                                                   /*  初始化 spi 组件库           */
    if (pSpiFuncs) {
        API_SpiAdapterCreate("/bus/spi/0", pSpiFuncs);                  /*  创建 spi0 总线适配器        */
    }
...
static VOID  halDevInit (VOID)
{
    ...
    SIO_CHAN    *psio1 = sioChan16C550Create(1);                        /*  创建串口 1 通道             */
    ttyDevCreate("/dev/ttyS1", psio1, 30, 50);                          /*  add    tty   device         */

    SIO_CHAN    *psio2 = sioChan16C550Create(2);                        /*  创建串口 2 通道             */
    ttyDevCreate("/dev/ttyS2", psio2, 30, 50);                          /*  add    tty   device         */

    SIO_CHAN    *psio3 = sioChan16C550Create(3);                        /*  创建串口 3 通道             */
    ttyDevCreate("/dev/ttyS3", psio3, 30, 50);                          /*  add    tty   device         */

    SIO_CHAN    *psio4 = sioChan16C550Create(4);                        /*  创建串口 4 通道             */
    ttyDevCreate("/dev/ttyS4", psio4, 30, 50);                          /*  add    tty   device         */
#if 0
    SIO_CHAN    *psio5 = sioChan16C550Create(5);                        /*  创建串口 5 通道             */
    ...
    ads7846DevCreate("/dev/input/touch0");                              /*  ads7846 触摸屏设备          */
...
static VOID  halNetifAttch (VOID)
{
    ...
    lib_srand((pnetif->hwaddr[0] << 24) |
              (pnetif->hwaddr[1] << 16) |
              (pnetif->hwaddr[2] <<  8) |
              (pnetif->hwaddr[3]));                                     /*  可以用 mac 设置随机数种子   */
